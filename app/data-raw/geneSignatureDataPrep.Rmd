...
```{r, read-in}
ge <- readRDS("../data-raw/hipc_2020-09-28_gene_expression_v31-recreated_template.RDS")
ge <- ge[7:nrow(ge), 2:ncol(ge)]
keepCols <- c("target_pathogen", "response_component_original", "tissue_type",
              "response_behavior", "time_point", "time_point_units", "publication_reference")
...
ge$timepoint_concat <- paste(ge$updated_timepoint, ge$updated_timepoint_units, "-")
```

```{r, save}
keepCols <- c("pubmed_titles",
...
geneSignatures <- ge[ !duplicated(ge$uid), ] # lose about half the rows?
saveRDS(geneSignatures, "../data/geneSignatures.rds")
```